class LifeGame {
    //フィールドのサイズ(番兵は含めない)
    private _fieldSize: { x: number, y: number } = { x: 64, y: 36 };
    //実際のセル(Dead = 0, Alive > 0)
    private _cells: number[][];

    constructor(option?: { fieldSize?: { x: number, y: number } }) {
        //もし引数があれば設定
        if (option) {
            //フィールドサイズの設定
            if (option.fieldSize) {
     ...
     */
    setCell(x: number, y: number, aliveFlag: boolean = true) {
        //もし指定した座標が範囲外ならエラー
